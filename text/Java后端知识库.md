...
- [HTTP 缓存 vs Redis 缓存：两层缓存不是一回事](#http-缓存-vs-redis-缓存两层缓存不是一回事)
- [状态码怎么选：401 / 403 / 429 / 503 的边界（附"HTTP 全 200"的取舍）](#状态码怎么选401--403--429--503-的边界附http-全-200的取舍)
- [Cookie/Session vs JWT：无状态的代价是什么](#cookiesession-vs-jwt无状态的代价是什么)
- [HTTPS 握手：为什么用混合加密、证书到底证明了什么](#https-握手为什么用混合加密证书到底证明了什么)

**操作系统**
- [进程 vs 线程的本质区别](#进程-vs-线程的本质区别)
- [进程切换为什么比线程切换贵（页表 / TLB）](#进程切换为什么比线程切换贵页表--tlb)
- [Java 线程与内核线程的映射关系](#java-线程与内核线程的映射关系)
- [虚拟内存到底解决了什么问题](#虚拟内存到底解决了什么问题)
- [缺页中断与页面置换（为什么内核不用精确 LRU）](#缺页中断与页面置换为什么内核不用精确-lru)
...
### HTTP 缓存 vs Redis 缓存：两层缓存不是一回事

**问题**：项目里既用 HTTP 缓存（`Cache-Control`/`ETag`）又用 Redis 缓存，它们是同一个东西吗？该用哪个？

**答案**：**不是一回事，而且位置、失效方式、解决的目标都不同。** 一个项目里两者可以并存，但**只有 HTTP 缓存能挡住"请求根本不到服务器"**。

| | **HTTP 缓存** | **Redis 缓存** |
| --- | --- | --- |
| 存在哪 | **客户端**（浏览器，或 CDN/代理等中间节点） | **服务端**（应用和数据库之间） |
| 谁去读 | 浏览器自己判断，**可能一个请求都不发** | 应用代码主动查（`GET learning:article:detail:1`） |
| 命中时 | 请求**根本不到你的服务器**（省的是网络 + 你的 CPU） | 请求到了服务器，只是**不查 DB**（省的是 DB） |
| 失效手段 | `max-age` 到期、`ETag`/`Last-Modified` 协商（**服务端改不了客户端已缓存的副本**） | 服务端主动 `DEL`，**立即生效** |
| 精度 | 依赖客户端时钟/实现，只能到秒级或协商 | 完全由服务端控制 |
| 典型内容 | 静态资源（JS/CSS/图片）、公开的、可缓存的响应 | 任意业务数据、**用户相关的、需要立即失效的** |

**⭐ 关键区别：能不能主动失效。**

- **HTTP 缓存**：资源一旦被浏览器（或 CDN）缓存在 `max-age` 内，**服务端没有任何办法让它立刻失效**。所以：
  - 生产上给带 hash 的静态资源配 `max-age=31536000, immutable`（改代码就换文件名，新 URL 天然不冲突）；
  - **`index.html` 必须 `no-cache`** —— 否则用户永远拿不到新版入口，也就引用不到新的 JS/CSS；
  - **绝不能让错误响应被缓存**（CDN 把一个"密码错误"缓存下来分发给所有人 = 事故）。
- **Redis 缓存**：服务端 `DEL` 一下立刻生效。你项目里文章更新/删除就靠这个把脏数据清掉。

**为什么你项目里 HTTP 缓存没配**：`/article/{id}` 这类接口**返回的是用户相关的数据**（还带 token 鉴权），一旦被中间节点缓存，就会**把 A 的数据发给 B**。所以这类接口的响应要么不加 `Cache-Control`（默认不缓存），要么显式 `Cache-Control: no-store`。**"用户相关的响应不能进共享缓存"是一条安全底线。**

**本项目实例**：`ArticleServiceImpl` 的 Cache Aside（服务端 Redis 层）+ `md/Redis缓存设计文档.md`。至于 HTTP 层，项目目前**没有配任何 `Cache-Control`**，这对一个带鉴权的 JSON API 来说是**正确的默认行为**（不该被共享缓存）。

**面试怎么答**：先说"这是两层，位置不同" —— HTTP 缓存是**客户端/中间节点**缓存，能挡住请求；Redis 是**服务端**缓存，挡的是数据库查询。然后抛出最值钱的那句：**"HTTP 缓存服务端无法主动失效，所以静态资源靠文件名 hash 换 URL，而用户相关的数据必须走服务端缓存（Redis），因为它需要立刻失效。"** 追问方向：强缓存和协商缓存的区别（答：强缓存不发请求，协商缓存发请求但可能只回 304 空体）、`no-cache` 和 `no-store` 的区别（答：`no-cache` 是"可以存但每次要问服务器"，`no-store` 是"根本不许存"）、`ETag` 为什么比 `Last-Modified` 准（答：时间戳秒级 + "改了又改回来"会误判，ETag 基于内容哈希）。

---

### 状态码怎么选：401 / 403 / 429 / 503 的边界（附"HTTP 全 200"的取舍）

**问题**：什么时候返 401、什么时候 403、什么时候 503？有人主张"HTTP 全返 200、业务码放 body"，这样做对不对？

**答案**：**四个码语义完全不同，混用会让前端做出错误动作**；而"全 200"是**有明确代价的工程取舍**，不是对错问题。

| code | 语义 | 前端正确反应 | 用错的后果 |
| --- | --- | --- | --- |
| **401** | **未认证**：没带凭证 / 凭证过期 / 已登出 / 类型不对 | 用 refreshToken 换新；换不到就**跳登录页** | — |
| **403** | **无权限**：身份有效，但不能动这个资源 | 提示"无权操作"，**不要跳登录页** | 用 401 代替 → 用户被误踢下线 |
| **429** | **限流** | 提示"操作太频繁"，按 `Retry-After` 等待 | 用 401 代替 → 用户一次限流就被踢下线 |
| **503** | **服务端暂时不可用**（依赖挂了，如 Redis） | 提示稍后重试，**保留凭证** | 用 401 代替 → **Redis 抖一下全站用户被踢下线** |

**⭐ 最容易错的是 503 用成 401。** 场景：Redis 挂了，鉴权时查不了黑名单。这时"查不到"和"调用方凭证无效"是两件完全不同的事：
- 返 401 → 前端清 token 跳登录页 → 用户重登还是失败（登录也要写 Redis）→ 体验是"整个系统坏了"；
- 返 503 → 前端提示"服务稍后重试"、**保留 token** → 正确。

**"HTTP 全返 200"的取舍**（这个项目选的就是它）：

| 收益 | 代价 |
| --- | --- |
| 前端只在拦截器里判 `body.code`，一套逻辑 | **监控失明**：Prometheus/APM/SLB 都按 HTTP 状态码统计错误率，全 200 则"错误率永远是 0%"，告警形同虚设 |
| 天然避开 Tomcat 的 Whitelabel HTML 错误页（避免前端 JSON 解析崩溃） | **DevTools 灾难**：Network 面板全绿，排查要一个个点开看 |
| 早期能绕过某些运营商/代理对非 200 响应的劫持 | **网关熔断失效**：Sentinel/Gateway 按"非 200 比例"熔断，全 200 则下游崩了也继续转发 → 雪崩 |
| 业务错误码可以无限细分 | **CDN 可能缓存错误响应**并分发给后续用户 |

> **现代最佳实践是"双层"**：**传输层用真实 HTTP 状态码**（未登录 401、服务器崩 500、传参错 400），**响应体里再放细粒度业务码**（`{"code":10002,"message":"密码必须包含大写字母"}`）。既让基础设施能识别，又满足前端细粒度提示。

**本项目实例**（这是个少见的**四码都用上了**的教学样本）：
- `JwtInterceptor` 写 401（`Result.unauthorized`），**区分**"未登录/格式错误/token 无效/登录已过期"四种文案；
- `AuthorizationInterceptor` 写 **403**（`Result.forbidden`）—— 早期错用过 401，被改掉了（见「权限判断写反」那条的教训）；
- `RateLimitInterceptor` 写 **429**（`Result.overLimit`）+ `X-RateLimit-*` / `Retry-After` 头；
- `TokenServiceImpl.isRevoked` 抛 **503**（`BusinessException(503)`）—— 黑名单查不到时 fail-closed；
- 全部遵循项目铁律：**HTTP 状态码恒 200，业务码在 body 的 `code` 里**（连超限和 503 也是 200）。

**面试怎么答**：先给一句话区分 —— **"401 是'你是谁不知道'，403 是'知道你是谁但不让你动'，429 是'你太快了'，503 是'我这边暂时不行'。"** 然后主动讲 503 那条：**"Redis 挂了导致黑名单查不到时，我返 503 而不是 401 —— 因为 401 会让前端清 token 跳登录页，而用户的凭证其实是好的。"** 最后可以谈"全 200"：**"我们知道它的代价（监控和网关熔断失效），所以是知情的选择；现代做法是传输层用真实状态码 + body 里放业务码。"**

---

### Cookie/Session vs JWT：无状态的代价是什么

**问题**：Session 方案不是挺好吗？JWT 到底解决了什么问题、又付出了什么代价？

**答案**：两者都在解决 **HTTP 无状态** 这一个问题，但**状态的存放位置不同**，于是优缺点正好互补。

| | **Cookie + Session** | **JWT（本项目）** |
| --- | --- | --- |
| 状态存哪 | **服务端**（内存 / Redis），客户端只有一个 `SessionId` | **客户端**（token 自带全部信息），服务端不存 |
| 服务端能否立即踢人 | ✅ **能**：删掉 Session 即可 | ❌ **不能**：token 签发后服务端改不了它（这是 JWT 最大的代价） |
| 水平扩容 | ⚠️ 多实例要**共享 Session**（所以要塞 Redis，或做粘性会话） | ✅ 天然无状态，加机器就行 |
| 跨域/跨端 | ⚠️ Cookie 有同源策略、CSRF 问题 | ✅ 放 `Authorization` 头，移动端/多端友好 |
| 传输体积 | 小（只有一个 SessionId） | 大（每次请求都带完整 payload，且 base64 后更膨胀） |
| 载荷可读性 | 服务端数据，客户端看不到 | ⚠️ **payload 是明文 base64，任何人都能解**（但改不了，有签名） |

**⭐ JWT 的核心代价：撤销难。** 这就是为什么"双 Token + 黑名单"这套设计存在：

```
access（短，30 分钟）  → 泄露窗口可控，登出时进【黑名单】立即失效
refresh（长，7 天）    → 存 Redis、可删、可轮转，是"真正可撤销的那一环"
```

**你项目里对这个代价的处理**（这是能讲的工程细节）：
1. **access 短有效期**把"撤不掉"的伤害限制在 30 分钟内；
2. **黑名单**（`learning:token:blacklist:{sha256}`）让登出**立即生效**，TTL 取 token 剩余有效期而非固定值（自动清理）；
3. **refresh 存 Redis 用 userId 做 key** → 新登录顶掉旧的 → 天然单端登录；
4. **改角色/降级时删 refresh key** → 让旧凭证**无法续期**（虽然 access 还会残留 ≤30 分钟）。

**三个必须记住的安全点**：
1. **JWT payload 是明文**（你解码 token 时亲眼看到 `role:1`）→ **绝不放密码、身份证、手机号**；
2. **`HttpOnly` Cookie 能防 XSS 偷 token**（JS 读不到）；放 `localStorage` 则 JS 可读 → **有 XSS 就等于 token 泄露**；
3. **但 Cookie 会带来 CSRF**（浏览器自动携带）→ 需要 `SameSite=Lax/Strict` 或 CSRF Token。**"放哪里"是在 XSS 和 CSRF 之间选一个，没有免费的方案。**

**本项目实例**：`TokenServiceImpl`（issue/refresh/logout/isRevoked）、`JwtUtils`（签发带 `jti`/`type`/`role`）、`md/JWT双Token实现设计文档.md`。

**面试怎么答**：先给结论 —— **"Session 把状态放服务端，能随时踢人但要共享存储；JWT 把状态放客户端，天然无状态但撤销要靠额外机制。"** 然后主动展开代价与补救：**"JWT 改不了已签发的 token，所以我用'短 access + Redis 存 refresh + 黑名单'来补：access 只有 30 分钟限制伤害，登出时 access 进黑名单立即失效、refresh 直接删。改角色时也删 refresh，让它无法续期。"** 追问方向：token 放 Cookie 还是 localStorage（答：HttpOnly Cookie 防 XSS 但有 CSRF，localStorage 反之，是取舍）、JWT 能不能存敏感信息（答：不能，payload 明文可解）、为什么 refresh 要存服务端（答：因为它是"可撤销"这一职责的承担者）。

---

### HTTPS 握手：为什么用混合加密、证书到底证明了什么

**问题**：HTTPS 为什么"非对称 + 对称"混着用？证书是拿来加密的吗？

**答案**：**证书不是用来加密数据的，是用来证明"这把公钥属于这个域名"的** —— 即**防中间人**。而"混合加密"是为性能和安全的折中。

**四步握手（TLS 1.2，以 ECDHE 为例，2 个 RTT）**：

```
① Client Hello     客户端：支持的 TLS 版本 / 密码套件 / 客户端随机数 Rc / SNI(要访问哪个域名)
② Server Hello     服务端：选定套件 / 服务端随机数 Rs / 【证书链】/ ECDHE 参数(用证书私钥签名)
                   ↓ 客户端【验证证书链】
③ Client Key Exchange  客户端发自己的 ECDHE 参数
   + Finished          双方各自算出 Pre-Master Secret → 结合 Rc+Rs 推出【会话密钥】
④ Server Finished  服务端也回 Change Cipher Spec + Finished，握手结束
                   ↓ 之后全部用【对称密钥】加密传输
```

**⭐ 为什么必须混合**：
- **非对称加密（RSA/ECDHE）**：能安全地在**不可信信道**上协商出密钥，但**慢几个数量级**，不可能拿来加密全部流量；
- **对称加密（AES）**：快，但**双方得先有同一把密钥**，而在明文信道上直接传密钥 = 把钥匙和锁一起寄出去；
- 所以：**用非对称"安全地协商出对称密钥"，之后切对称**。非对称只用一次。

**⭐ 证书验证链解决的问题**：客户端怎么确认"对面真是 baidu.com，不是隔壁老王搭的代理"？
```
服务器证书（含公钥、域名、有效期）
   ↓ 用【中间 CA 的公钥】解密证书里的签名 → 得到摘要 B
   ↓ 自己把证书明文做 SHA-256 → 得到摘要 A
   A == B  ⇒  ① 内容没被篡改  ② 确实是这个 CA 签发的
   ↓ 递归往上：中间 CA 由【根 CA】担保
   ↓ 根 CA 自签名，且【预装在操作系统/浏览器的根证书库里】
```
**"信任"最终来源于操作系统/浏览器出厂预装的根证书 —— 不是数学，是社会信任的固化。** 另外还会做：**域名匹配（SAN）、有效期检查、吊销检查（CRL/OCSP）**。

**TLS 1.3 的改进**：握手从 2-RTT 压到 **1-RTT**（Client Hello 就带上自己的 ECDHE 参数"猜"一把），支持 0-RTT；**强制前向安全**（废除 RSA 密钥交换，只留 ECDHE）→ **服务器私钥将来泄露，也无法解密历史抓包**。

**和你项目的关联**：
1. **HTTPS 下 token 就不用再加密了** —— 传输层已经保护；
2. **但 token 的内容仍然不该放敏感信息**（见上一条：payload 是明文 base64）；
...
**问题**：为什么说"进程是资源分配的基本单位，线程是 CPU 调度的基本单位"？线程哪些东西私有、哪些共享？

**答案**：**根本差异在于有没有独立的虚拟地址空间（页表）。**

- **进程有独立的虚拟地址空间** —— 每个进程一张**页表**，操作系统让它以为自己独享整个内存；
- **同一进程内的线程共享同一张页表和虚拟地址空间** —— 只是各自有独立的栈和寄存器。

**⭐ Linux 视角的加分点**：内核其实**不区分进程和线程**，两者都用 `task_struct` 表示，调度器只调度 `task_struct`。区别只在于：

- 两个 `task_struct` 的 `mm_struct`（虚拟内存描述符）**指向同一个** → 它们是同进程的**线程**；
- 指向**不同**的 → 是不同**进程**。
...
| 线程私有 | 线程共享 |
| --- | --- |
| 程序计数器 PC、寄存器组 | 代码段（Text） |
| 栈（局部变量、调用栈帧） | 数据段 / BSS（全局变量、静态变量） |
| 线程局部存储 TLS | 堆（`new` / `malloc` 出来的） |
| | 打开的文件描述符表 |
| | 信号处理方式 |

> ⚠️ **踩分点**：线程栈"私有"只是**逻辑隔离**，物理上**没有硬件保护** —— 一个线程只要拿到另一个线程栈上变量的指针，就能直接读写它。

**本项目实例**：这个"共享还是独立"的划分在 JVM 里一一对应 —— 线程私有 = 虚拟机栈 / 本地方法栈 / PC 寄存器；线程共享 = 堆 / 元空间。所以 `StackOverflowError` 永远是**线程私有**问题，`OutOfMemoryError: Java heap space` 是**线程共享**问题。

**面试怎么答**：一句话定调 —— **"进程和线程的本质区别是有没有独立的页表：进程有，线程共享所在进程的。"** 然后按"私有 / 共享清单 → Linux 的 `task_struct` 视角 → 优缺点的工程含义"展开。主动补一个踩分点：**线程栈的私有只是逻辑隔离、没有硬件保护**。追问方向：为什么不都用进程（答：切换要换页表、通信要走 IPC，代价高）、为什么不都用线程（答：一个线程段错误整个进程就挂，Chrome 用多进程换稳定性）、`mm_struct` 是什么（答：描述虚拟地址空间的结构体，是判断"进程还是线程"的判据）。

---

### 进程切换为什么比线程切换贵（页表 / TLB）

**问题**：进程切换比线程切换"重"，到底重在哪？

**答案**：**重不在"切换动作"本身，而在切换之后要"还债"。**

**切了什么**：

- **线程切换（同进程内）**：切 PC、通用寄存器、状态寄存器、**栈指针** → 换到另一个线程的私有栈
- **进程切换**：在上面基础上**多一步 —— 改写 `CR3` 寄存器**（指向新进程的顶级页表物理基址）

**为什么改写 `CR3` 会导致性能雪崩**：

1. **TLB 被刷掉（TLB Flush）**：TLB 缓存 `VPN → PFN` 映射。改了 `CR3`，老进程的映射全废 → 早期 CPU 直接**清空整个 TLB**。而多级页表一次翻译要访存 4~5 次，**没有 TLB 兜底就只能靠硬件 Page Walker 逐级查表**，每次访存慢上百倍。
2. **Cache 被污染（Cold Cache）**：老进程的热点代码/数据对新进程毫无意义，切换后 Cache 命中率极低 → 大量 Cache Miss → 流水线停顿（Pipeline Stall）。

**量化对比**（面试给出量级，说服力拉满）：

| 代价类型 | 内容 | 量级 |
| --- | --- | --- |
| **直接代价** | 陷入内核、保存/恢复寄存器、更新 PCB | **几十 ~ 几百纳秒** |
| **间接代价** | TLB 重新预热 + Cache 重新填满 | **数微秒甚至更久** ← **真正的大头** |

**现代优化：PCID（Process-Context Identifier）**
给每个 TLB 表项打上"属于哪个进程"的标签，改 `CR3` 时**不清空 TLB**，只比对标签；切回原进程时它的 TLB 可能**依然有效** —— 大幅缓解进程切换的冷启动惩罚（ARM 上叫 ASID）。

**本项目实例**：这条是"线程池为什么要限线程数"的底层依据 —— 线程一多，CPU 时间大量消耗在上下文切换的**间接代价**上。所以 CPU 密集型设 N 或 N+1，IO 密集型设 2N 或更高。

**面试怎么答**：一句话定调 —— **"进程切换比线程切换多一步改写 `CR3`，代价是 TLB 失效 + Cache 污染；真正贵的是切换之后的 TLB/Cache 预热，不是切换本身。"** 三段展开：切了什么 → 为什么雪崩（TLB / Cache）→ 现代怎么优化（PCID）。追问方向：TLB 是干什么的（答：页表映射的硬件缓存，基于局部性原理，命中就不必走多级页表）、为什么线程切换不刷 TLB（答：同进程线程共享页表，`CR3` 没变）、那线程切换就免费吗（答：不，寄存器和栈还是要换，只是没有 TLB/Cache 的额外惩罚）。

---

### Java 线程与内核线程的映射关系

**问题**：Java 线程和操作系统线程是什么关系？"Java 线程很重"到底重在哪？

**答案**：**Java 线程 = 内核级线程，1:1 映射。** `Thread.start()` 底层是 `JVM_StartThread` → `pthread_create`，每个 Java 线程对应一个内核 `task_struct`。

**先讲三种线程模型的框架**：

| 模型 | 对应关系 | 调度者 | 优点 / 致命伤 |
| --- | --- | --- | --- |
| **用户级 ULT** | M:1 | 用户态运行时 | 切换极快（不陷内核）；**但①用不了多核 ②一个阻塞全进程卡死** |
| **内核级 KLT** | 1:1 | OS 调度器 | 多核友好、单个阻塞不影响其他；**但创建/切换都要系统调用** |
| **混合 M:N** | M:N | 运行时 + 内核 | 理想模型（Go 的 GMP）；调度复杂 |

**"Java 线程很重"的三座大山**：

1. **栈内存大**：`-Xss` 默认 **1MB**，1000 个线程光栈就 1GB → 撑不起 C10K
2. **创建/销毁贵**：必须走 `pthread_create`，发生用户态→内核态切换 + 分配内核栈
3. **调度开销大**：内核是**通用公平调度器**，它不知道你的线程是在做运算还是在等 Redis；线程一多，CPU 大量耗在切换上

**JDK 21 虚拟线程（Project Loom）革了谁的命**：

- **重回 M:N**：大量虚拟线程多路复用映射到少量**载体线程（Carrier Thread）**
- **栈在堆上**：不再是固定 1MB，而是 JVM 堆里的小对象，初始仅几百字节 → 单机可开百万级
- **遇到阻塞 IO 时"卸载"**：JVM 在**用户态**把虚拟线程从载体线程卸载（Unmount），载体线程立刻去跑别的虚拟线程；IO 就绪后再挂载（Mount）回来
- **最大价值是编码范式**：继续写直观的同步阻塞代码，却拿到异步的吞吐 —— 不用再写回调地狱

**本项目实例**：Learning 的线程池参数就受这条约束 —— 线程池必须限界（禁止 `Executors.newCachedThreadPool()`）。

**面试怎么答**：一句话定调 —— **"Java 线程是 1:1 的内核级线程，`Thread.start()` 最终调 `pthread_create`。"** 然后按"三模型框架 → Java 为什么重（1MB 栈 / 系统调用 / 内核盲调度）→ JDK 21 虚拟线程怎么解决（M:N + 栈在堆上 + 阻塞时卸载）"展开。追问方向：虚拟线程能提升计算性能吗（答：不能，它只解决 IO 等待时的线程占用，CPU 密集型用平台线程更合适）、虚拟线程有什么坑（答：`synchronized` 块内阻塞会 pin 住载体线程、ThreadLocal 滥用会放大内存）、为什么不能无脑开线程（答：1MB 栈 + 切换的间接代价）。

---

### 虚拟内存到底解决了什么问题

**问题**：为什么需要虚拟地址？直接让程序用物理地址不行吗？

**答案**：直接操作物理内存有**三个致命问题**，虚拟内存正好一一解决：

| 直接操作物理内存的问题 | 虚拟内存的解法 |
| --- | --- |
| **① 没有隔离**：A 的野指针能覆盖 B 甚至内核的数据 | **隔离**：每个进程独立虚拟地址空间 + 独立页表，相同的虚拟地址映射到不同物理内存 |
| **② 空间不够**：程序必须完整装进物理内存才能跑 | **超额分配 + Swap**：只给真正访问到的部分分配物理内存；不够就把不活跃的页换到磁盘 |
| **③ 地址不确定**：每次加载地址都可能变，要逐条改指令（静态重定位） | **简化链接与加载**：统一虚拟内存布局（代码段固定从低地址排、栈从高地址往下长），加载器原样丢进去即可 |

**本质一句话**：虚拟内存是**软硬件协同的一层地址映射抽象**，把"程序员眼中的逻辑内存"和"机器底层的物理内存"**彻底解耦**。
> "所有问题都可以通过增加一个间接层来解决。"
...
| --- | --- | --- |
| 划分依据 | 逻辑（代码 / 数据 / 栈） | 物理（固定 4KB） |
| 大小 | **不固定** | **固定** |
| 碎片 | **外部碎片**（要"内存紧缩"，代价极高） | **内部碎片**（每进程最多浪费 < 1 页，可接受） |
| 硬件实现 | 相对简单 | 依赖 MMU 与复杂页表 |

**现实是段页式**：x86 硬件上先分段再分页，但 **Linux 把所有段的基址设为 0、界限设为最大**，把硬件分段"架空"（平面模型），**所有保护 / 隔离都下放到页表**。软件层面仍保留"段"的概念（Linux 的 `vm_area_struct`，即 VMA）。

**本项目实例**：JVM 堆本质就是进程虚拟地址空间里的一大段 VMA —— 所以同一台机器上两个 Spring Boot 进程各自的堆互不影响，一个 OOM 也不会拖垮另一个。

**面试怎么答**：一句话定调 —— **"虚拟内存用一层地址映射把逻辑内存和物理内存解耦，换来隔离、超额分配、简化加载这三件事。"** 然后**按"三个噩梦 → 三个解法"一一对应展开**（这样答结构最清晰），再补"实现上是分页胜出，Linux 用平面模型架空了分段"。追问方向：物理内存不够了怎么办（答：Swap 换出 + 页面置换）、分页和分段哪个好（答：分页无外部碎片且硬件友好所以胜出；分段权限控制直观所以软件层保留 VMA）、虚拟地址空间多大（答：由架构决定，x86-64 常见 48 位有效地址）。

---

### 缺页中断与页面置换（为什么内核不用精确 LRU）

**问题**：虚拟内存能"超额分配"，那访问到不在物理内存的数据会怎样？内存满了该淘汰谁？

**答案**：触发**缺页中断（Page Fault，`#PF`，14 号中断）**，内核四步处理：

1. **合法性检查**：非法地址（空指针、写只读段）→ 直接发 `SIGSEGV` 干掉进程；合法地址说明只是被换出或还没读进来
2. **找空闲物理页框**：有空闲直接用；**满了就执行置换算法**选出牺牲页 ——
   - **脏页（被改过）必须先写回磁盘**；**干净页（如代码段）直接丢弃**
3. **磁盘 I/O 换入**目标数据
4. **修复页表项并重试**：填好 PFN、把存在位置 `1`，**重新执行刚才那条引发缺页的指令**（这次 MMU 命中，程序毫无感知）

**四种置换算法**：

| 算法 | 规则 | 评价 |
| --- | --- | --- |
| **OPT** | 淘汰未来最久不用的 | 上帝视角，**无法实现**，只作理论上限 |
| **FIFO** | 谁先来谁先走 | 无视局部性；**有 Belady 异常** |
| **LRU** | 淘汰过去最久没用的 | 性能接近 OPT，但内核做不到精确版 |
| **Clock** | 环形链表 + 访问位，给第二次机会 | **LRU 的硬件友好近似**，真实系统在用 |

**⭐ Belady 异常**：直觉上"内存越大缺页越少"，但 **FIFO 会出现"内存变大、缺页反而变多"**。
**LRU / OPT 不会有** —— 它们属于**栈式算法**：分配 N 个页框时保留的页面集合，永远是 N+1 个时的**子集**，内存变大只会让热集合只增不减。

**⭐⭐ 为什么内核做不到精确 LRU**（这是能区分候选人的点）：

1. **访存频率是天文数字**：每秒数千万次内存读写
2. **精确 LRU 要求每次访存都更新**：要么改时间戳，要么把节点从双向链表摘下来移到表头（还要加锁）→ **每次访存都执行几十条软件指令，性能当场暴跌百倍**
3. **硬件能给的极限**：MMU 在查页表命中时，**最多顺手把一个 `Accessed Bit` 置 1** —— 它不可能维护双向链表

**Linux 的落地：双链 Clock（Active / Inactive）**

- 新页先进 **Inactive（冷区）**；后台线程 `kswapd` 定期巡检
- Inactive 里访问位被置 1 的 → **升迁到 Active（热区）**；Active 里久未访问的 → **降级回 Inactive**
- 真淘汰时**只从 Inactive 尾部挑**
- 附带好处：**能防御全表扫描攻击**（一个进程读 10GB 大文件，不会一次性把热点缓存全挤掉）

**本项目实例**：两处现成的对应 —— ① **Redis 的内存淘汰策略**（LRU / LFU / random / ttl）就是同一套问题在缓存侧的解法（见知识库 Redis 相关条目）；② **LeetCode #146 LRU 缓存**（哈希表 + 双向链表）是**应用层**的标准答案。

**面试怎么答**：一句话定调 —— **"访问到不在内存的页会触发缺页中断，内核四步处理：合法性检查 → 找页框（不够就置换）→ 换入 → 修复页表并重试。"** 置换算法按"OPT 是上限、FIFO 有 Belady 异常、LRU 是理想、Clock 是落地"讲。**主动放这个降维打击**：

> "**应用层**做 LRU 就是哈希表 + 双向链表（LeetCode #146）；但**内核**做不到精确 LRU —— 每次访存都加锁移动链表节点，CPU 承受不起。硬件最多给一个 `Accessed Bit`，所以 Linux 用 **Clock + Active/Inactive 双链**做伪 LRU，在开销和准确率之间取平衡。"

这一句就能把你从"刷题的人"变成"懂系统设计的人"。追问方向：脏页和干净页的区别（答：脏页必须写回、干净页可直接丢）、Belady 异常为什么只有 FIFO 有（答：它只记进入顺序、与访问频率无关，且不是栈式算法）、Swap 抖动是什么（答：频繁换入换出，说明物理内存严重不足，表现为系统卡死）。
