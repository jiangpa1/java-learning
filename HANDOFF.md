...
**待办**（详见 `LearningHANDOFF.md` 第八节；**2026-09-19 第二轮复核后已收敛**）：
1. ~~文档一致性收尾~~ —— **已完成**（四表 DDL 已用 `SHOW CREATE TABLE` 与库对齐、逻辑删除验收项改正、列表页滞后落文档，并新增 `README.md`）
2. **Docker 部署**（简历差异点）—— 仓库里目前**没有任何 Dockerfile / docker-compose / CI 文件**
3. 继续铺单测（优先 `UserServiceImpl` 的权限判断 —— `!A || !B` 写反时大部分用例还是绿的，最该有测试；其次 `ArticleServiceImpl` 的缓存降级）
4. 低优先：统一提示语（**昵称兜底文案 4 处不一致**）、`JwtProperties` 改用 `@EnableConfigurationProperties`；**可选**：补 Maven Wrapper（`mvnw`，现在 `.mvn` 是空目录，克隆下来只能靠 IDEA 构建）
...
**项目侧**：
1. ~~文档一致性收尾~~ —— **已完成**（2026-09-19 第二轮，含新增 `README.md`）
