...
| 9.19 | 计算机网络：HTTP报文/方法语义(安全与幂等)/状态码(401vs403vs429vs503)/HTTP缓存(强缓存vs协商缓存)/Cookie-Session与JWT对比、HTTPS握手与证书链(混合加密原理)；项目实战：用户列表分页、请求体解析异常(500→400)、文章分类关联校验；**单元测试入门**(JUnit5+Mockito，51个用例覆盖降级方向与已修缺陷)；用户模块接口文档按实现重写；知识库补4条 | \#56 |
